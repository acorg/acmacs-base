...
{
  public:
    constexpr Color() : color{0xFF00FF} {}
    constexpr (const Color&) = default;
    template <typename Uint, typename std::enable_if_t<std::is_integral_v<Uint>>* = nullptr> constexpr Color(Uint aColor) : color_(static_cast<uint32_t>(aColor)) {}
    explicit Color(std::string_view src) { from_string(src); }
    ...
    template <typename Uint, typename std::enable_if_t<std::is_integral_v<Uint>>* = nullptr> constexpr Color& operator=(Uint aColor) { color_ = static_cast<uint32_t>(aColor); return *this; }

    constexpr bool operator == (const Color& aColor) const { return color_ == aColor.color_; }
    constexpr bool operator != (const Color& aColor) const { return ! operator==(aColor); }
    constexpr bool operator < (const Color& aColor) const { return color_ < aColor.color_; }

    constexpr double alpha() const { return double(0xFF - ((color_ >> 24) & 0xFF)) / 255.0; } // 0.0 - transparent, 1.0 - opaque
    constexpr double red() const { return double((color_ >> 16) & 0xFF) / 255.0; }
    constexpr double green() const { return double((color_ >> 8) & 0xFF) / 255.0; }
    constexpr double blue() const { return double(color_ & 0xFF) / 255.0; }

    constexpr uint32_t alphaI() const { return static_cast<uint32_t>((color_ >> 24) & 0xFF); }
    constexpr uint32_t redI() const { return static_cast<uint32_t>((color_ >> 16) & 0xFF); }
    constexpr uint32_t greenI() const { return static_cast<uint32_t>((color_ >> 8) & 0xFF); }
    constexpr uint32_t blueI() const { return static_cast<uint32_t>(color_ & 0xFF); }
    constexpr void alphaI(uint32_t v) { color_ = (color_ & 0xFFFFFF) | ((v & 0xFF) << 24); }
    constexpr uint32_t rgbI() const { return static_cast<uint32_t>(color_ & 0xFFFFFF); }
