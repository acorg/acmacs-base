    ...
    class SymbolHandler
    {
     public:
        ...
        [[noreturn]] inline void unexpected(std::string_view::value_type aSymbol, Parser& aParser) const
            {
                throw rjson::parse_error(aParser.line(), aParser.column(), std::string{"unexpected symbol: '"} + aSymbol + "' (" + acmacs::to_hex_string(static_cast<unsigned char>(aSymbol), acmacs::ShowBase, acmacs::Uppercase) + ")");
            }
