...
namespace to_json
{
    namespace v1
    {
        ...
        enum Options { option_default, ignore_empty_values };
        ...
        template <typename T> inline std::string value(T&& aValue)
        {
            ...
            if constexpr (decay_equiv_v<T, std::string_view>) {
                return "\"" + std::string(aValue) + "\"";
            }
            else if constexpr (is_string_v<T>) {
                return "\""s + aValue + '"';
            }
            else if constexpr (decay_equiv_v<T, raw>) {
                return std::move(aValue);
            }
            else if constexpr (decay_equiv_v<T, bool>) {
                return aValue ? "true" : "false";
            }
            else if constexpr (decay_equiv_v<T, std::nullptr_t>) {
                return "null";
            }
            else if constexpr (decay_equiv_v<T, null_t>) {
                return "null";
            }
            else if constexpr (decay_equiv_v<T, undefined_t>) {
                return "undefined";
            }
            else if constexpr (std::numeric_limits<std::decay_t<T>>::is_integer) {
                return std::to_string(aValue);
            }
            else if constexpr (decay_equiv_v<T, double>) {
                return acmacs::to_string(aValue);
            }
            else if constexpr (decay_equiv_v<T, float>) {
                return std::to_string(aValue);
            }
            // else if constexpr (std::is_floating_point_v<std::decay_t<T>>)       { return acmacs::to_string(aValue); }
            else {
                static_assert(std::is_same<int, std::decay_t<T>>::value, "use std::move?"); /* compilation fails trying to instantiate this type */
            }
        }
        ...
        namespace internal
        {
            template <typename Arg> inline std::string object1(Arg&& arg) { return to_json::v1::object(std::begin(arg), std::end(arg)); }
        } // namespace internal

        template <typename... Args> inline std::string object(Args&&... args)
        {
            if constexpr (sizeof...(args) == 1) {
                return internal::object1(std::forward<Args>(args)...); // for to_json::v1::object(const std::map<Key, Value>& aMap)
            }
        ...
        template <typename... Args> inline std::string object(Options options, Args&&... args) { return internal::object_append(std::string{}, options, std::forward<Args>(args)...); }

        template <typename... Args> inline std::string object_append(std::string target, Args&&... args) { return internal::join(target, object(std::forward<Args>(args)...)); }

        template <typename... Args> inline std::string object_prepend(std::string target, Args&&... args) { return internal::join(object(std::forward<Args>(args)...), target); }

        // ----------------------------------------------------------------------

        template <typename... Args> inline std::string array(Args&&... args) { return internal::array_append(std::string{}, std::forward<Args>(args)...); }

        template <typename Iterator, typename = acmacs::sfinae::iterator_t<Iterator>> inline std::string array(Iterator first, Iterator last)
        {
            std::string target = "[]";
            for (; first != last; ++first) {
        ...
        template <typename Iterator, typename Transformer, typename = acmacs::sfinae::iterator_t<Iterator>> inline std::string array(Iterator first, Iterator last, Transformer transformer)
        {
            std::string target = "[]";
            for (; first != last; ++first) {
        ...
        template <typename... Args> inline std::string array_append(std::string target, Args&&... args) { return internal::join(target, array(std::forward<Args>(args)...)); }

        template <typename... Args> inline std::string array_prepend(std::string target, Args&&... args) { return internal::join(array(std::forward<Args>(args)...), target); }

        template <typename T> inline std::string value(const std::vector<T>& aVector) { return array(std::begin(aVector), std::end(aVector)); }
