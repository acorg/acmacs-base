...
namespace acmacs::argv::inline v2
{
    class argv;
    using str = std::string_view;
    using str_array = std::vector<std::string_view>;

    struct desc : public std::string_view
    {
        using std::string_view::string_view;
    };
    struct arg_name : public std::string_view
    {
        using std::string_view::string_view;
    };
    enum mandatory { mandatory };
    template <typename T> struct dflt
    {
        dflt(T&& val) : value{std::move(val)} {}
        T value;
    ...
    namespace detail
    {
        ...
        template <> std::string to_string(const str_array& source) noexcept { return fmt::format("\"{}\"", string::join(acmacs::string::join_sep_t{"\" \""}, source)); }
    ...
    template <> constexpr option<bool>::operator const bool&() const
    {
        if (value_.has_value())
            return *value_;
        return detail::false_;
    }

    template <> void option<str_array>::add(std::string_view arg)
    {
        if (!value_)
            value_ = str_array{arg};
        else
